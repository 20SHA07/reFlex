...
type VoiceHistoryItem = {
  type: string;
  role: string;
  content: Array<{ transcript?: string | null; text?: string }>;
};
type VoiceSessionEvents = {
  on(event: "history_updated", listener: (history: VoiceHistoryItem[]) => void): void;
...
export default function VoicePage() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const events = session as unknown as VoiceSessionEvents;
      events.on("history_updated", (history) => {
        const lines = history
